"""
Gap 2a: Metric Reconstruction via MDS and Gromov Delta
=======================================================
Tests the geometry of the emergent distance metric via:
  1. Classical MDS eigenvalue spectrum (negative eigenvalues = non-Euclidean)
  2. Gromov delta hyperbolicity (delta/diameter: 0=tree, small=hyperbolic, large=flat)

Result: delta = 0 exactly — perfect tree metric. MDS gives all positive eigenvalues.
Conclusion: the bulk IS a tree metric. Boundary test needed for AdS signal (see boundary_entropy_exact.py).

Run: python reconstruct_metric.py
Requires: results/depth{8,10}_ensemble_summary.json
